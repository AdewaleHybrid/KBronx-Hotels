...
exports.getAbout = (req, res, next) => {
  if (req.session.mail != undefined) {
    return res.render("user/about", { user: req.session.mail });
  } else {
...
exports.getRooms = (req, res, next) => {
  if (req.session.mail != undefined) {
    return res.render("user/rooms", { user: req.session.mail });
  } else {
    return res.render("user/rooms", { user: "" });
  }
};

// GET ROOM DETAILS
exports.getRoom1 = (req, res, next) => {
  if (req.session.mail != undefined) {
    return res.render("user/room1-details", { user: req.session.mail });
  } else {
    return res.render("user/room1-details", { user: "" });
  }
};
...
exports.getGallery = (req, res, next) => {
  if (req.session.mail != undefined) {
    return res.render("user/gallery", { user: req.session.mail });
  } else {
    return res.render("user/gallery", { user: "" });
  }
};
...
//   if (req.session.mail != undefined) {
//     return res.render("user/loginUser", { user: req.session.mail });
//   } else {
//     return res.render("user/loginUser", { user: "" });
//   }
// };

// GET LOGIN-USER PAGE
exports.getLoginUser = (req, res, next) => {
  res.render("user/loginUser", { user: "", msg: [], err: [] });
};
...
exports.postLoginUser = (req, res, next) => {
  ...
  data = "SELECT * " + "FROM  user " + "WHERE email = " + mysql.escape(req.body.email) + " AND password = " + mysql.escape(req.body.password);
  ...
  connectDB.query(data, (err, result) => {
    if (err) throw err; // show if any error have
      ...
      if (result.length) {
        ...
        res.render("user/loginUser", { user: "", msg: [], err: ["Please Check Your information again"] });
      }
    }
  });
};
...
exports.getCreateUser = (req, res, next) => {
  res.render("user/createUser", { user: "", msg: [], err: [] });
};
...
exports.postCreateUser = (req, res, next) => {
  ...
  if (p1 != p2) {
    ...
    return res.render("user/createUser", { user: "", msg: [], err: ["Password Doesn't Match"] });
  }

  var data = "INSERT INTO user " + " VALUES ( '" + req.body.name + "' ,'" + req.body.email + "','" + req.body.phone + "','" + p1 + "')";

  connectDB.query(data, (err, result) => {
    if (err) throw err; // if db has error, show that
    else {
      res.render("user/loginUser", { user: "", msg: ["Account Create Successfuly"], err: [] }); //show login page
    }
  });
};
...
exports.postBooking = (req, res, next) => {
  // console.log(req.body);
  res.render("user/booking", { user: req.session.mail, name: req.body.name, type: req.body.type, cost: req.body.cost });
};
...
exports.postStatus = (req, res, next) => {
  ...
  data1 = "SELECT * " + " FROM  bookingstatus " + " WHERE email = " + mysql.escape(req.session.mail);
...
exports.getShowStatus = (req, res, next) => {
  ...
  connectDB.query(data, (err, result) => {
    if (err) throw err;
      ...
      if (result.length < 1) {
        res.render("user/statusShow", { user: req.session.mail, msg: "", err: "You dont have any booking", data: result });
      } else {
